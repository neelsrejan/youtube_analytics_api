            ...
            for i in range(len(filters)):
                if filters[i] == "country" or filters[i] == "continent" or filters[i] == "subContinent":
                    if filters[i] == "country":
                        ...
                        response_df.to_csv(os.path.join(os.getcwd(), f"{self.channel_name}_data", f"{date.today()}", "video_reports", "csv", "top_videos_regional", f"{required_dimension},{filters[i]},{sort_by}.csv"), index=False)
                        response_df.to_excel(os.path.join(os.getcwd(), f"{self.channel_name}_data", f"{date.today()}", "video_reports", "excel", "top_videos_regional", f"{required_dimension},{filters[i]},{sort_by}.xlsx"), index=False)
                    ...
                    if len(response["rows"]) != 0:
                        for row in response["rows"]:
                            data.append(row)
                    response_df = pd.DataFrame(data=data, columns=col_names)
                    response_df.to_csv(os.path.join(os.getcwd(), f"{self.channel_name}_data", f"{date.today()}", "video_reports", "csv", "top_videos_regional", f"{required_dimension},{sort_by}.csv"), index=False)
                    response_df.to_excel(os.path.join(os.getcwd(), f"{self.channel_name}_data", f"{date.today()}", "video_reports", "excel", "top_videos_regional", f"{required_dimension},{sort_by}.xlsx"), index=False)
        return
