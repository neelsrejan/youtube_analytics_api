...
class User_Activity_In_US_Over_Time(Auth):

    def user_activity_in_US_over_time(self):
                    ...
                    if i == 0 and j != 0:
                        if filters_1[i] == "province": 
                            if filters_2[j] == "video":
                                ...
                                response_df.to_csv(os.path.join(os.getcwd(), f"{self.channel_name}_data", f"{self.date_time}", "raw", "video_reports", "csv", "user_activity_in_US_over_time", f"{dimension},{filters_1[i]},{filters_2[j]}.csv"), index=False)
                                response_df.to_excel(os.path.join(os.getcwd(), f"{self.channel_name}_data", f"{self.date_time}", "raw", "video_reports", "excel", "user_activity_in_US_over_time", f"{dimension},{filters_1[i]},{filters_2[j]}.xlsx"), index=False)
                            ...
                            for province in self.provinces:
                                ...
                                if len(response["rows"]) != 0:
                                    for row in response["rows"]:
                                        data.append(row)
                            response_df = pd.DataFrame(data=data, columns=col_names)
                            response_df.to_csv(os.path.join(os.getcwd(), f"{self.channel_name}_data", f"{self.date_time}", "raw", "video_reports", "csv", "user_activity_in_US_over_time", f"{dimension},{filters_1[i]}.csv"), index=False)
                            response_df.to_excel(os.path.join(os.getcwd(), f"{self.channel_name}_data", f"{self.date_time}", "raw", "video_reports", "excel", "user_activity_in_US_over_time", f"{dimension},{filters_1[i]}.xlsx"), index=False)
        return
